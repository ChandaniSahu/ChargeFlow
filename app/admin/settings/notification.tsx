  ...
  const getNotificationIcon = (status: string) => {
    switch (status) {
      case "sent":
        return {
          icon: <Bell size={20} className="group-hover:text-white" />,
          className: "bg-[#dcfce7] text-[#16a34a] group-hover:bg-[#16a34a]",
        };

      case "warning":
        return {
          icon: (
            <AlertTriangle size={20} className="group-hover:text-white" />
          ),
          className: "bg-[#fff0e5] text-[#ff8000] group-hover:bg-[#ff8000]",
        };

      case "failed":
        return {
          icon: <X size={20} className="group-hover:text-white" />,
          className: "bg-[#ffe4e6] text-[#fb2c2f] group-hover:bg-[#fb2c2f]",
        };

      default:
        return {
          icon: <Send size={20} className="group-hover:text-white" />,
          className: "bg-[#f2f2f2] text-[#757575] group-hover:bg-[#757575]",
        };
    }
  };
  ...
  const getNotificationStatus = (status: string) => {
    switch (status) {
      ...
      case "warning":
        return {
          icon: (
            <AlertTriangle size={14} className="group-hover:text-white" />
          ),
          className: "bg-[#fff0e5] text-[#ff8000] group-hover:bg-[#ff8000]",
        };
    ...
    <>
      <div className="font-inter space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h2 className="text-[20px] sm:text-[24px] font-medium">
              Notification Settings
            </h2>

            <p className="text-[16px] sm:text-[18px] text-[#7C7C7C]">
              Configure email and push notifications
            </p>
          </div>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 bg-[#30EF0A] hover:bg-green-500 cursor-pointer text-white px-5 sm:px-6 py-2 sm:py-3 rounded-[10px] text-[16px] sm:text-[20px] font-medium w-full sm:w-auto"
          >
            <Plus size={20} />
            Create New Notification
          </button>
        ...
        <div>
          ...
          <h3 className="text-[22px] sm:text-[24px] font-medium mb-4">
            Notification
          </h3>

          <div className="border border-[#C9C8C8] rounded-[16px] p-4 sm:p-6 bg-white">

            <h4 className="text-[18px] sm:text-[21px] font-medium mb-4 text-[#364153]">
              Active Automations
            </h4>
            ...
            <div className="space-y-4">
              ...
              {automations.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="border border-[#C9C8C8] rounded-[12px] py-3 px-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4"
                  >
                    ...
                    <div>
                      ...
                      <h4 className="flex gap-1 text-[16px] font-medium text-[#364153]">
                        ...
                        {item.scheduleType && (
                          <span>
                            {item.scheduleType.charAt(0).toUpperCase() +
                              item.scheduleType.slice(1)}
                          </span>
                        )}
                      </h4>

                      <p className="text-[14px] sm:text-[16px] text-[#7C7C7C]">
                        Start:{" "}
                      ...
                      <p className="flex flex-wrap gap-4 text-[14px] sm:text-[16px] text-[#7C7C7C]">
                        <span>
                          Time:{" "}
                          <span className="text-[#364153]">
        ...
        <div className="border border-[#C9C8C8] rounded-[16px] p-4 sm:p-6 bg-white">
          ...
          <h3 className="text-[20px] sm:text-[22px] font-medium mb-4">
            Recent Notifications
          </h3>
          ...
          <div className="space-y-4">
            ...
            {recent.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="border border-[#C9C8C8] rounded-[14px] p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3"
                >
                  ...
                  <div className="flex sm:flex-row  sm:items-center sm:justify-center sm:gap-5 items-end justify-between gap-2 w-full sm:w-auto">

                    <div className="text-[14px] text-[#7C7C7C]">
                      <p>{formatTime(item.date)}</p>
                      <p className="text-[12px]">{formatDate(item.date)}</p>
                    </div>

                    <div
                      className={`flex items-center gap-1 px-3 py-1 text-[12px] rounded-md ${statusData.className}`}
                    >
                      {statusData.icon}
        ...
        <div>
          ...
          <h3 className="flex items-center gap-2 text-[20px] sm:text-[24px] font-medium">
            <Bell size={22} /> Admin Notifications
          </h3>
          ...
          <div className="mt-3">
            ...
            <Toggle
              ...
              enabled={adminNotifications.highPrioritySupportTickets}
              onToggle={() =>
                handleToggleChange("highPrioritySupportTickets")
              }
            />
            ...
            <Toggle
              title="Charger Offline Alerts"
              subtitle="Get notified when any charger becomes offline"
              enabled={adminNotifications.chargerOfflineAlerts}
              onToggle={() => handleToggleChange("chargerOfflineAlerts")}
            />
  ...
  return (
    <div className="flex items-center justify-between py-3">
      ...
      <div className="max-w-[75%]">
        ...
        <h4 className="text-[16px] sm:text-[18px] font-medium text-[#364153]">
          {title}
        </h4>
