      ...
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 px-6 md:grid-cols-3">
        {infoData.map((item, i) => (
          ...
          >
            ...
            <h3 className="mb-1 text-[20px] font-semibold font-inter">{item.title}</h3>
            <p className="mb-4 text-[12px] text-gray-600 font-roboto">{item.des}</p>
            <button className="font-roboto rounded-full bg-[#38EF0A] px-4 py-2 text-sm text-white text-[20px] font-semibold flex flex-wrap items-center justify-center gap-2 cursor-pointer hover:bg-[#32c700] transition-colors duration-300 ease-in-out">
              {item.btn} <ArrowIcon/>
